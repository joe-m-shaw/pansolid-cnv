...
The sample information from the DNA Database was extracted using the erbb2_val_dna_db_queries.R script which uses an ODBC connection. The information was exported as csv files so that this Quarto document should render without the user needing an ODBC connection to the DNA Database.
...
NHS numbers can be pulled off DLMS although annoyingly for two lab numbers they aren't included (21015264 and 24008372). The NHS numbers are on different samples for the same patients, so I've added the NHS numbers to an additional csv file and added them in below. This avoids hard-coding patient-identifiable information into this Quarto.

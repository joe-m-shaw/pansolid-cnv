...
if (nrow(new_ws_amp_gene_results) == 0 &
  ...
  rm(new_ws_stdev_results)
    ...
    write.csv(pansolid_data, here::here("data/pansolid_v2_cnv_service_results.csv"),
              row.names = FALSE)
...
```{r}
#| label: summary-gene-amps
#| echo: FALSE

summary_table <- live_results_mod |> 
...
gene_fold_changes_plot <- ggplot(live_results_mod, aes(x = gene, y = max_region_fold_change)) +

...
write.csv(x = wgs_pansolid_comparison, file = paste0(outputs_folder, "tables/",
                                                     format(Sys.time(),
                                                            "%Y_%m_%d_%H_%M_%S"),
                                                     "_",
                                                     "wgs_pansolid_comparison.csv"),
          row.names = FALSE)
...
```{r}
#| label: sample-count-by-gene
#| echo: FALSE

wgs_sample_gene_count <- wgs_reformatted_with_ids |> 
  filter(nhs_no_clean %in% wgs_pansolid_comparison$nhsno) |> 
  mutate(wgs_code = case_when(
    
    wgs_result %in% c("No gain detected", "No loss detected") ~"wgs_negative",
    
    wgs_result %in% c("Gain detected", "Loss detected") ~"wgs_positive"
    )) |> 
  group_by(gene, wgs_code) |> 
  summarise(total = n()) |> 
...
seraseq_genes <- c("AKT2", "BRAF", "EGFR", "ERBB2", "FGFR3", "KIT", "KRAS",
                   "MET", "MYC", "MYCN", "NTRK1", "PIK3CA")

seracare_gene_count <- rbind(del_gene_table, amp_gene_table) |> 
  mutate(seracare_positive = case_when(
    gene %in% seraseq_genes ~3,
    TRUE ~0),
    
    seracare_negative = case_when(
      gene %in% seraseq_genes ~1,
      TRUE ~0))

ddpcr_gene_count <- rbind(del_gene_table, amp_gene_table) |> 
  mutate(ddpcr_positive = case_when(
    gene == "ERBB2" ~11,
    gene == "MYC" ~27,
    gene == "EGFR" ~3,
    gene == "MET" ~3,
...
sample_summary <- wgs_sample_gene_count |> 
  inner_join(seracare_gene_count, by = "gene") |> 
  inner_join(ddpcr_gene_count, by = "gene") |> 
  mutate(total_negative = sum(wgs_negative, seracare_negative, ddpcr_negative),
         total_positive = sum(wgs_positive, seracare_positive, ddpcr_positive)
         ) |> 
  filter(gene %in% amp_gene_table$gene) |> 
  select(gene, total_negative, total_positive)

sum(sample_summary$total_negative)
sum(sample_summary$total_positive)

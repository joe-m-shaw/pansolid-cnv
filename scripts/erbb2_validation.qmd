...
clc_controls <- read_excel(path = here::here("data/control_data/clc_controls.xlsx")) |> 
...
lod_names <- c("SeraSeq_LOD_25_percent", "SeraSeq_LOD_12_percent",
               "SeraSeq_LOD_6_percent", "SeraSeq_LOD_0_percent")
...
ggplot(results_with_tumour_source, aes(x = reorder(gene, fold_change),
                                 y = fold_change)) +
...
plot_fold_change_vs_ddpcr_copy_number <- ddpcr_pansolid |> 
  ...
  ggplot(aes(x = max_region_fold_change*2, y = cnv)) +
...
copies_3 <- make_fold_change_plot(df = erbb2_pansolid_call_details |> 
...
sample_21011525_results <- erbb2_pansolid_call_details |> 
  filter(labno == "21011525")
...
```{r}
#| label: all-target-calls
#| eval: FALSE
#| message: FALSE
#| include: FALSE

filepath <- "S:/central shared/Genetics/NGS/Bioinformatics/1_Pan-solid-Cancer/CNV/QC/"

files <- list.files(filepath, full.names = TRUE, pattern = ".xlsx")

consistent_files <- files[-30]

x <- consistent_files |>
  map(\(consistent_files) read_clc_target_calls(
    file = consistent_files
  )) |>
  list_rbind() 

x |> 
  filter(name %in% grep(pattern = "ERBB2", ignore.case = TRUE,
     x = unique(x$name), value = TRUE)) |> 
  filter(!duplicated(labno)) |> 
  ggplot(aes(x = reorder(labno_suffix_worksheet, regional_fold_change), 
...
```{r}
#| label: all-target-calls-repeats
#| eval: FALSE

x |> 
  select(labno, labno_suffix, labno_suffix_worksheet, patient_name,
         name, regional_fold_change) |> 
  distinct() |> 
  filter(name %in% grep(pattern = "ERBB2", ignore.case = TRUE,
     x = unique(x$name), value = TRUE)) |>  

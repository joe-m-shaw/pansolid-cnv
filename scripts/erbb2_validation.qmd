...
```{r}
#| label: qiaseq-primer-lengths
#| echo: FALSE

plot_qiaseq_primer_length <- grch38_primer_coordinates |> 
  mutate(size = end - start) |>  
  ggplot(aes(x = size, y = )) +
  geom_histogram(binwidth = 1) +
  labs(title = "PanSolid primer lengths",
       x = "Primer length (bp)", y = "Number of primers") +
  theme_bw()

plot_qiaseq_primer_length
...
pos_cnv_results <-  processed_files |> 
  map(\(processed_files) read_pos_cnv_results(file = processed_files)) |> 
  list_rbind()

all_amp_gene_results <- processed_files |> 
...
stopifnot(unique(pos_cnv_results$file) == processed_files)

stopifnot(stdev_results$file == processed_files)

stopifnot(unique(all_amp_gene_results$file) == processed_files)

# Check full gene table has been read
stopifnot(nrow(all_amp_gene_results) / 9 == length(processed_files))

```

```{r}
#| label: calls-per-gene
#| echo: FALSE

calls_per_gene <- pos_cnv_results |> 
  filter(gene != "no positive calls") |> 
  group_by(labno_suffix_worksheet) |> 
  count(gene)

ggplot(calls_per_gene, aes(x = n, y = )) +
  geom_bar() +
  stat_bin(aes(label=..count..), binwidth = 1, vjust = -0.5, 
  ...
  theme_bw() +
  facet_wrap(~gene) +
  labs(x = "Amplification calls per gene",
       y = "Number of samples")
...
```{r}
#| label: fold-change-threshold-check
#| include: FALSE

current_threshold <- 2.9

proposed_threshold <- 2.8

fc_threshold_df <- all_amp_gene_results |> 
  mutate(result_current_threshold = case_when(
    
      max_region_fold_change >= current_threshold ~"amplification",
      
      max_region_fold_change < current_threshold ~"normal"),
    
    result_proposed_threshold = case_when(
      
      max_region_fold_change >= proposed_threshold ~"amplification",
      
      max_region_fold_change < proposed_threshold ~"normal"))

fc_threshold_df_long <- fc_threshold_df |> 
  pivot_longer(cols = c(result_current_threshold, result_proposed_threshold),
               names_to = "category",
               values_to = "result")

fc_threshold_comparison <- fc_threshold_df_long |> 
  group_by(gene, category) |> 
  count(result) |> 
  pivot_wider(id_cols = gene,
              names_from = c(category, result),
              values_from = n) |> 
  mutate(difference_normal =  result_proposed_threshold_normal - result_current_threshold_normal,
         difference_amp =  result_proposed_threshold_amplification - result_current_threshold_amplification)

csv_timestamp(fc_threshold_comparison)

fc_threshold_change_plot <- ggplot(all_amp_gene_results |> 
         filter(max_region_fold_change > 0 & max_region_fold_change < 5), 
       aes(x = gene, y = max_region_fold_change)) +
  geom_jitter(pch = 21, size = 2, aes(fill = pansolid_call)) +
  theme_bw() +
  geom_hline(yintercept = current_threshold, linetype = "dashed") +
  geom_hline(yintercept = proposed_threshold, linetype = "dashed") 

fc_threshold_change_plot
...
clc_controls <- control_files |> 
  map(\(control_files) filename_to_df(file = control_files)) |> 
  list_rbind()
...
all_patient_samples <- unique(stdev_results$labno)

table_sample_extractions <- erbb2_sample_extraction |> 
  filter(labno %in% all_patient_samples) |> 
...
table_sample_types <- erbb2_sample_types |> 
  filter(labno %in% all_patient_samples & !labno %in% seracare_ids) |> 
  count(tissue_type) |> 
  arrange(desc(n)) |> 
...
erbb2_tissue_sources_coded <- erbb2_tissue_sources |> 
  mutate(tissue_code = case_when(
    ...
    TRUE ~tissue_source)) |> 
  filter(tissue_code != "seracare control")

table_tumour_sources <- erbb2_tissue_sources_coded |> 
   filter(labno %in% all_patient_samples) |>
  count(tissue_code) |> 
  rename(Total = n,
         "Tumour source" = tissue_code) |> 
  arrange(desc(Total))
...
Samples were tested across `r nrow(erbb2_worksheets)` between `r min(erbb2_worksheets$date)` and `r max(erbb2_worksheets$date)`.
...
erbb2_pansolid_region_results <- all_amp_gene_results |> 
  filter(gene == "ERBB2") |> 
  filter(!duplicated(labno))
...
erbb2_pansolid_call_details <- pos_cnv_results |> 
  filter(gene == "ERBB2") |> 
  filter(!duplicated(labno))

```

Out of `r length(all_patient_samples)` samples, there were `r length(unique(erbb2_pansolid_call_details$labno))` with ERBB2 amplifications detected, with fold changes ranging from `r round(min(erbb2_pansolid_call_details$fold_change), 1)` to `r round(max(erbb2_pansolid_call_details$fold_change), 1)`.
...
table_results_by_tumour_type <- erbb2_pansolid_region_results |> 
  filter(!labno %in% seracare_ids) |> 
  left_join(erbb2_tissue_sources_coded, by = "labno") |> 
...
```{r}
#| label: fold-change-vs-stdev
#| echo: FALSE

gene_results_with_sd <- all_amp_gene_results |> 
...
fc_max_limit <- max(gene_results_with_sd$fold_change) + 5

fc_min_limit <- min(gene_results_with_sd$fold_change) - 5

sd_max_limit <- max(gene_results_with_sd$st_dev_signal_adjusted_log2_ratios) + 0.4

sd_min_limit <- min(gene_results_with_sd$st_dev_signal_adjusted_log2_ratios) - 0.4

ggplot(gene_results_with_sd, aes(x = st_dev_signal_adjusted_log2_ratios,
                                          y = fold_change)) +
  ...
  geom_point(data = gene_results_with_sd |> 
               filter(fold_change >= 2.9), fill = safe_red, size = 3,
             pch = 21) +
  facet_wrap(~gene)
...
```{r}
#| label: make-individual-plots
#| eval: FALSE
#| include: FALSE

draw_fc_sd_plot <- function(id) {
  
  x <- gene_results_with_sd |> 
    filter(labno_suffix_worksheet == id)
  
  plot <- ggplot(x, aes(x = st_dev_signal_adjusted_log2_ratios,
                                          y = fold_change)) +
    geom_point(size = 3, pch = 21, alpha = 0.4, aes(fill = gene)) +
    theme_bw() +
    geom_hline(yintercept = fold_change_threshold, linetype = "dashed") +
    xlim(sd_min_limit, sd_max_limit) +
    ylim(fc_min_limit, fc_max_limit) +
    labs(title = str_c("Sample: ", id))
...
for (i in unique(gene_results_with_sd$labno_suffix_worksheet)) {
  
  new_plot <- draw_fc_sd_plot(i)
  
  fc_sd_plots <- list(fc_sd_plots, new_plot)
  
  rm(new_plot)
  
}

ggexport(plotlist = fc_sd_plots, filename = here::here("outputs/plots/sd_plot_list.pdf"),
         width=15, height=8, res=300)

```

```{r}
#| label: call-size-vs-stdev
#| eval: FALSE
#| include: FALSE

pos_cnv_results_sd <- pos_cnv_results |> 
  filter(gene != "no positive calls") |> 
  left_join(stdev_results |> 
              select(labno_suffix_worksheet, 
                     st_dev_signal_adjusted_log2_ratios,
...
ggplot(pos_cnv_results_sd, aes(x = st_dev_signal_adjusted_log2_ratios,
                               y = cnv_length)) +
  ...
  facet_wrap(~gene) +
  scale_y_log10(limits = c(1000, 1000000000)) +
  xlim(0, 1.25) +
  labs(y = "CNV length (bp)")
...
results_with_tumour_source <- gene_results_with_sd |> 
  left_join(erbb2_tissue_sources_coded, by = "labno")

plot_fold_change_tumour_source <- ggplot(results_with_tumour_source, aes(x = reorder(gene, fold_change),
                                 y = fold_change)) +
  geom_jitter(pch = 21, size = 2, alpha = 0.5, aes(fill = tissue_code)) +
  geom_hline(yintercept = fold_change_threshold, linetype = "dashed") +
  theme_bw() +
...
plot_fold_change_tumour_source
  
```

```{r}
#| label: calls-vs-stdev
#| echo: FALSE

call_results_with_sd <- pos_cnv_results |> 
  left_join(stdev_results |> 
              select(labno_suffix_worksheet, 
                     st_dev_signal_adjusted_log2_ratios,
                     st_dev_signal_adjusted_fold_change), by = "labno_suffix_worksheet")

ggplot(call_results_with_sd, aes(x = st_dev_signal_adjusted_log2_ratios,
                                          y = fold_change)) +
...
ddpcr_pansolid <- ddpcr_erbb2 |> 
  ...
  left_join(erbb2_pansolid_region_results, by = "labno") |> 
  mutate(pansolid_result = case_when(
    
    labno %in% c(biobank_ids, control_ids) ~"pansolid normal control",
    
    pansolid_call == "normal result" ~"pansolid normal result",
    
    pansolid_call == "amplification" ~"pansolid amplification"),
...
The percentage agreements between NGS and ddPCR are:

Overall percentage agreement: `r table_erbb2_results_ngs_vs_ddpcr[[2]]`%.

Positive percentage agreement: `r table_erbb2_results_ngs_vs_ddpcr[[3]]`%.

Negative percentage agreement: `r table_erbb2_results_ngs_vs_ddpcr[[4]]`%.
...
plot_fold_change_vs_ddpcr_copy_number <- ddpcr_pansolid |> 
  filter(!is.na(max_region_fold_change)) |> 
  ...
  scale_x_continuous(limits = c(-20, 85), breaks = c(-20, 0, 20, 40, 60, 80)) +
...
```{r}
#| label: standard-deviation-of-repeats
#| echo: FALSE
...
number_replicates <- length(unique(repeat_results$labno_suffix_worksheet))

number_labnos <- length(unique(repeat_results$labno))

make_variation_plot <- function(df = repeat_results, 
                                y_axis,
                                yaxis_label,
                                fill_variable,
                                threshold,
  ...
  plot <- ggplot(df, aes(x = worksheet, y = {{ y_axis }})) +
    geom_point(pch = 21, size = 3, 
               aes(fill = {{ fill_variable }}), alpha = 0.4) +
    ...
    ylim({{ ymin }}, {{ ymax }}) +
              ...
              labs(x = "", y = yaxis_label,
                   title = "Variation in repeated samples",
                   subtitle = str_c("Number of samples: ", 
                                    number_labnos, 
...
make_variation_plot(y_axis = max_region_fold_change, 
                    fill_variable = pansolid_call,
                    threshold = fold_change_threshold,
                    yaxis_label = "ERBB2 region max fold change",
                    ymin = -20,
                    ymax = 50)

make_variation_plot(y_axis = st_dev_signal_adjusted_log2_ratios,
                    yaxis_label = "Standard deviation Log2Ratio",
                    fill_variable = qc_check,

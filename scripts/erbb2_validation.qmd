...
Exported tables and plots have the prefix "plot" or "table" and are named exactly as they appear in the validation document for ease of reference.
...
fc_vs_tcc <- data.frame(tcc = seq(0, 100, by = 0.1)) |> 
  mutate(target_copies = calculate_target_copies(fold_change = fold_change_threshold,
                                                 tcc_percent = tcc))
plot_fold_change_detection_limit <- fc_vs_tcc |> 
...
all_sample_info <- all_samples |> 
  left_join(sample_extraction, by = "labno") |> 
  left_join(sample_types, by = "labno") |> 
  rename(extraction_method = method_name)

table_patient_sample_types <- all_sample_info |> 
  filter(sample_group == "Samples tested using the CLC pipeline for ERBB2") |> 
  count(tissue_type) |> 
  arrange(desc(n))

knitr::kable(table_patient_sample_types)

csv_timestamp(table_patient_sample_types)

table_normal_sample_types <- all_sample_info |> 
  filter(sample_group == "MSI normal samples used as normal controls") |> 
  count(tissue_type) |> 
  arrange(desc(n))

knitr::kable(table_normal_sample_types)

csv_timestamp(table_normal_sample_types)

table_sample_extractions <- all_sample_info |> 
  filter(sample_group == "Samples tested using the CLC pipeline for ERBB2") |> 
...
table_normal_extractions <- all_sample_info |> 
  filter(sample_group == "MSI normal samples used as normal controls") |> 
  count(extraction_method) |> 
  arrange(desc(n))

knitr::kable(table_normal_extractions)

csv_timestamp(table_normal_extractions)
...
sample_discode_ncc <- sample_tbl |> 
  ...
  pivot_longer(cols = -c(labno, comments, ncc_db),
               values_to = "discode") |> 
  left_join(discode |> 
...
table_tumour_sources <- sample_disease_codes_mod |> 
  filter(!duplicated(labno)) |> 
...
impact_of_hindiii_digestion <- rbind(ws138419, ws138579) |> 
...
knitr::kable(impact_of_hindiii_digestion)

csv_timestamp(impact_of_hindiii_digestion)
...
origin_check <- coarse_calls |> 
  filter(gene %in% c("ERBB2", "No calls")) |> 
  filter(!duplicated(labno)) |> 
  left_join(sample_disease_codes_mod, by = "labno") |> 
  filter(!is.na(discode_comment))

amp_sample_type_table <- origin_check |> 
  count(discode_comment, gene) |> 
  pivot_wider(id_cols = discode_comment, 
              values_from = n,
              names_from = gene)

knitr::kable(amp_sample_type_table)
...
table_erbb2_results_ngs_vs_ddpcr <- tribble(
  ~"x",      ~"x",            ~"Amplification",      ~"Normal", 
  "ddPCR",  "Amplification",  true_positives,    false_negatives,
  "x",      "Normal",         false_positives,   true_negatives)

knitr::kable(table_erbb2_results_ngs_vs_ddpcr)

csv_timestamp(table_erbb2_results_ngs_vs_ddpcr)
...
exon_plot <- ggplot(erbb2_cdk12 |> 
                      mutate(labno = "Genes"), aes(x = start, y = labno)) +
  geom_segment(aes(x = start, xend = end, 
                   y = labno, yend = labno),
               linewidth = 5) +
  geom_segment(aes(x = erbb2_start, xend = erbb2_end, 
...
table_pansolid_vs_core_panel_erbb2_results <- tribble(
  ~"x",      ~"x",            ~"Amplification",      ~"Normal", 
  "Core",  "Amplification",  true_positives,    false_negatives,
  "x",      "Normal",         false_positives,   true_negatives)

knitr::kable(table_pansolid_vs_core_panel_erbb2_results)

csv_timestamp(table_pansolid_vs_core_panel_erbb2_results)

...
cobas_controls_collated <- cobas_control_files |> 
  map(\(cobas_control_files) filename_to_df(file = cobas_control_files)) |> 
  list_rbind()

qiasymphony_controls_collated <- read_csv(here::here("data/controls/qiasymphony_controls.csv"),
                                          col_types = "c") |> 
  mutate(worksheet = "WS139890")
...
qs_tissue_sources <- read_excel(here::here("data/qiasymphony_normal_tissue_sources.xlsx"))
...
all_cases <- unique(stdev_results_collated$labno)

table_sample_extractions <- sample_extraction_details |> 
  filter(labno %in% all_cases &!labno %in% excluded_cases) |> 
...
knitr::kable(table_case_types)

csv_timestamp(table_case_types)
...
table_tumour_sources <- sample_tissue_sources_coded |> 
  filter(labno %in% all_cases) |>
...
cobas_controls <- cobas_controls_collated |> 
  add_dna_db_info()

qiasymphony_controls <- qiasymphony_controls_collated |> 
  add_dna_db_info()
...
seracare_names <- unique(grep(pattern = "seraseq",
                              x = stdev_results$patient_name, 
                              ignore.case = TRUE, value = TRUE))

biobank_names <- unique(grep(pattern = "biobank",
                              x = cobas_controls_collated$patient_name, 
                              ignore.case = TRUE, value = TRUE))

cobas_control_ids <- unique(cobas_controls_collated$labno)

qiasymphony_control_ids <- unique(qiasymphony_controls$labno)
...
all_sample_details <- stdev_results |> 
  select(labno, worksheet, pansolid_version, extraction_method) |> 
  rbind(cobas_controls |> 
          select(labno, worksheet, pansolid_version, extraction_method)) |> 
  rbind(qiasymphony_controls |> 
          select(labno, worksheet, pansolid_version, extraction_method)) |> 
  
  mutate(sample_group = case_when(
    
    labno %in% seracare_ids ~"SeraCare",
    
    labno %in% cobas_control_ids & labno %in% biobank_ids ~"Cobas MCRC biobank control",
    
    labno %in% cobas_control_ids & !labno %in% biobank_ids ~"Cobas MSI-normal control",
    
    labno %in% qiasymphony_control_ids ~"Qiasymphony control",
    
    TRUE ~"Patient case"),
    
    # "FFPE" means EZ1 extraction - see DOC907
    extraction_method = ifelse(extraction_method == "FFPE", "EZ1", extraction_method))

table_samples_tested <- all_sample_details |>
  filter(sample_group %in% c("SeraCare", "Patient case")) |> 
  group_by(sample_group, extraction_method, pansolid_version) |> 
  summarise(Samples = length(unique(labno)),
            Replicates = n()) |> 
  arrange(sample_group, extraction_method, desc(Replicates))

total_line <- data.frame(sample_group = "Total",
                         extraction_method = "",
                         pansolid_version = 0,
                         Samples = sum(table_samples_tested$Samples),
                         Replicates = sum(table_samples_tested$Replicates))

table_samples_tested_totals <- rbind(table_samples_tested, total_line)

knitr::kable(table_samples_tested_totals)

table_controls_tested <- all_sample_details |>
  filter(!sample_group %in% c("SeraCare", "Patient case")) |> 
  group_by(sample_group, extraction_method, pansolid_version) |> 
  summarise(Samples = length(unique(labno)),
            Replicates = n()) |> 
...
total_control_line <- data.frame(sample_group = "Total",
                         extraction_method = "",
                         pansolid_version = 0,
                         Samples = sum(table_controls_tested$Samples),
                         Replicates = sum(table_controls_tested$Replicates))

table_controls_tested_totals <- rbind(table_controls_tested, total_control_line)

knitr::kable(table_controls_tested_totals)

csv_timestamp(table_controls_tested_totals)

csv_timestamp(table_samples_tested_totals)
...
```{r}
#| label: calls-by-pansolid-version
#| include: FALSE

erbb2_pansolid_region_results |> 
  filter(!labno %in% seracare_ids) |> 
  group_by(pansolid_version) |> 
  count(pansolid_call)
...
erbb2_pansolid_region_results |> 
  filter(pansolid_version == 2) |>
  select(-file) |>  view()

colnames(ddpcr_pansolid)

ddpcr_pansolid |> 
  select(labno, patient_name, gene, cnv, max_region_fold_change, pansolid_call) |>  view()
...
all_amp_gene_results |> 
  filter(pansolid_version == 2 & extraction_method == "COBAS" & gene == "ERBB2") |>  
  select(-file) |>  view()
...
table_results_by_tumour_type <- erbb2_pansolid_region_results |> 
  filter(!labno %in% seracare_ids) |> 
  left_join(sample_tissue_sources_coded, by = "labno") |> 
...
ddpcr_normal <- ddpcr_pansolid |> 
  filter(labno %in% cobas_control_ids)

...
table_results_by_tumour_type <- erbb2_pansolid_region_results |> 
  ...
  count(tumour_source, pansolid_call) |> 
  arrange(desc(n)) |> 
  pivot_wider(id_cols = tumour_source,
...
plot_fold_change_tumour_source <- ggplot(results_with_tumour_source, aes(x = reorder(gene, fold_change),
                                 y = fold_change)) +
  geom_jitter(pch = 21, size = 2, alpha = 0.5, aes(fill = tumour_source)) +
  geom_hline(yintercept = fold_change_threshold, linetype = "dashed") +
  theme_bw() +
...
low_fc_samples <- results_with_tumour_source |> 
  filter(tumour_source == "Central nervous system" &
           fold_change < 5)

...
csv_timestamp(table_ncc_increments)
...
processed_files <- list.files(path = here::here("data/clc_processed_outputs"), 
                              full.names = TRUE,
                              pattern = ".xlsx")
...
csv_timestamp(table_cobas_results_by_tumour_type_total)
...
plot_erbb2_cobas_v1 <- make_labno_plot(df = erbb2_cobas_v1_factorise, 
                gene = "ERBB2",
                interval = 20000, 
                buffer = 5000, 
                yaxis = labno_suffix_worksheet_factor)

plot_all_erbb2_cobas_v1_triptych <- make_cnv_triptych(plot_erbb2_cobas_v1)

plot_all_erbb2_cobas_v1_triptych

plot_timestamp(plot_all_erbb2_cobas_v1_triptych, input_height = 18)
...
plot_erbb2_cobas_v1_zoom <- make_labno_plot(df = short_erbb2_calls, 
                      gene = "ERBB2",
                      interval = 20000, 
                      buffer = 5000,
                      yaxis = labno_suffix_worksheet_factor)

plot_erbb2_amplification_accuracy <- make_cnv_triptych(plot_erbb2_cobas_v1_zoom)

plot_erbb2_amplification_accuracy

plot_timestamp(plot_erbb2_amplification_accuracy, input_height = 15)
...
copies_3v1 <- make_fold_change_plot(df = pos_cnv_results |> 
...
copies_6v1 <- make_fold_change_plot(df = pos_cnv_results |> 
...
copies_12v1 <- make_fold_change_plot(df = pos_cnv_results |> 
...
plot_timestamp(plot_seracare_controls_erbb2_results_v1, input_width = 16)
...
copies_3v2 <- make_fold_change_plot(df = pos_cnv_results |> 
...
copies_6v2 <- make_fold_change_plot(df = pos_cnv_results |> 
...
copies_12v2 <- make_fold_change_plot(df = pos_cnv_results |> 
...
plot_timestamp(plot_seracare_controls_erbb2_v2, input_width = 16)
...
csv_timestamp(table_ngs_seracare_comparison)
...
csv_timestamp(table_ddpcr_seracare_comparison)
...
plot_timestamp(plot_signal_adjusted_noise_threshold)
...
Pooled standard deviation for intra-run results for fold change (repeatability): `r repeatability_fold_change_sd`

Pooled standard deviation for inter-run results for fold change (reproducibility): `r reproducibility_fold_change_sd`

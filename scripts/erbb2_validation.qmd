...
clc_controls_collated <- control_files |> 
...
all_patient_samples <- unique(stdev_results_collated$labno)

table_sample_extractions <- sample_extraction_details |> 
  filter(labno %in% all_patient_samples &!labno %in% excluded_samples) |> 
...
table_sample_types <- sample_types |> 
  filter(labno %in% all_patient_samples & !labno %in% excluded_samples) |> 
  count(tissue_type) |> 
  arrange(desc(n)) |> 
...
ggplot(sample_dna_concentrations, aes(x = reorder(labno, desc(stock_qubit)), 
                                      y = stock_qubit)) +
  geom_point() +
  theme(axis.text.x = element_blank())

max(sample_dna_concentrations$stock_qubit)

median(sample_dna_concentrations$stock_qubit)

min(sample_dna_concentrations$stock_qubit)
...
```{r}
#| label: add-extraction-and-pansolid-version
#| include: FALSE

# Add on extraction and PanSolid version to each table, to allow for easier
# analysis later on

pos_cnv_results <- pos_cnv_results_collated |>  
  filter(!labno %in% excluded_samples) |> 
  add_extraction_and_pansolid_version()
...
clc_controls <- clc_controls_collated |> 
  add_extraction_and_pansolid_version()
...
all_sample_details <- stdev_results |> 
  select(labno, suffix, worksheet, patient_name, pansolid_version, extraction_method) |> 
  rbind(clc_controls |> 
          select(labno, suffix, worksheet, patient_name, pansolid_version, extraction_method)) |> 
  
  mutate(sample_group = case_when(
    
    patient_name %in% seracare_names ~"SeraCare",
    
    patient_name %in% biobank_names ~"MCRC biobank",
    
    (labno %in% control_ids & !patient_name %in% biobank_names) ~"MSI-normal",
    
    TRUE ~"Patient FFPE samples"),

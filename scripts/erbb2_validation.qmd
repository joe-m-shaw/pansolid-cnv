...
table_ncc_increments <- fc_vs_ncc |> 
  ...
  mutate("Lowest number of theoretically detectable target copies" = round(target_copies, 1)) |>
...
source(here::here("functions/archived_functions.R"))

all_patient_files <- list.files(path = here::here("data/clc_outputs/"), full.names = TRUE, recursive = TRUE,
                pattern = "*.xlsx")
...
qc_data_with_ids |> 
  filter(st_dev_signal_adjusted_log2_ratios < 0.5 &
           percent_target_region_pos_cov_greaterorequal_138 < 75) |>  view()

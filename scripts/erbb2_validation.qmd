...
```{r}
#| label: remove-unnecessary-samples
#| include: FALSE

# Sample excluded because sample type is fresh frozen tissue
excluded_samples <- c("23034646")

pos_cnv_results <- pos_cnv_results_collated |>  
  filter(!labno %in% excluded_samples)
...
all_sample_details <- stdev_results |> 
  filter(labno != "23000087") |> 
  select(labno, suffix, worksheet) |> 
  rbind(clc_controls |> 
          select(labno, suffix, worksheet)) |> 
  left_join(erbb2_sample_extraction, by = "labno") |> 
  left_join(erbb2_worksheets, by = "worksheet") |> 
...
total_line <- data.frame(sample_group = "Total",
                         extraction_method = "",
                         pansolid_version = 0,
                         Samples = sum(x$Samples),
                         Replicates = sum(x$Replicates))

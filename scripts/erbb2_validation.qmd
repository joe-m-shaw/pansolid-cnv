...
case_details_with_groups <- stdev_results |> 
...
table_cases_tested <- case_details_with_groups |>
...
cases_total_line <- data.frame(sample_group = "Total",
                         extraction_method = "",
                         pansolid_version = 0,
...
table_controls_tested <- rbind(cobas_controls, qiasymphony_controls) |>
  ...
  mutate(extraction_method = ifelse(extraction_method == "FFPE", "EZ1", extraction_method)) |> 
  group_by(sample_subgroup, extraction_method, pansolid_version) |> 
  summarise(Samples = length(unique(labno))) |> 
...
ddpcr_erbb2 <- rbind(ws138579, ws138765) |> 
  ...
  # Add NHS number for comparison with PanSolidv2 in later step
  left_join(sample_nhs_no, by = "labno")
...
table_cobas_results_by_tumour_type <- erbb2_pansolid_region_results |> 
...
table_qiasymphony_results_by_tumour_type <- erbb2_pansolid_region_results |> 
  filter(extraction_method == "QIAsymphony_DNA_FFPE") |> 
  count(tissue_code, pansolid_call) |> 
  arrange(desc(n)) |> 
  pivot_wider(id_cols = tissue_code,
              names_from = pansolid_call,
...
colnames(erbb2_pansolid_region_results)

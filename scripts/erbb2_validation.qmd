...
table_calls_per_sample <- all_patient_calls |> 
  filter(gene == "ERBB2") |> 
  ...
  arrange(desc(fine)) |> 
  rename("ERBB2 calls - coarse" = coarse,
         "ERBB2 calls - fine" = fine)

knitr::kable(table_calls_per_sample)

```

```{r}
#| label: disproportionate-chromosome-coverage
#| echo: FALSE

ggplot(erbb2_coarse_pansolid, aes(x = reorder(labno_suffix_worksheet,
                                              desc(fold_change_adjusted)),
                                  y = fold_change_adjusted)) +
  geom_point(aes(colour = comments)) +
...
erbb2_calls_factorised <- erbb2_coarse_pansolid |> 
...
plot_all_erbb2_amplification_coordinates <- make_cnv_triptych(
  plot_xmin = erbb2_labno[[1]],
  plot_xmax = erbb2_labno[[2]],
  interval = erbb2_labno[[3]],
  main_plot = erbb2_labno[[4]],
  chromosome = erbb2_labno[[5]])
...
plot_erbb2_amplification_accuracy <- make_cnv_triptych(plot_xmin = erbb2_labno_zoom[[1]],
                  plot_xmax = erbb2_labno_zoom[[2]],
                  interval = erbb2_labno_zoom[[3]],
                  main_plot = erbb2_labno_zoom[[4]],
                  chromosome = erbb2_labno_zoom[[5]])
...
copies_3 <- make_fold_change_plot(df = all_patient_calls |> 
...
copies_6 <- make_fold_change_plot(df = all_patient_calls |> 
...
copies_12 <- make_fold_change_plot(df = all_patient_calls |> 
...
plot_variation_in_21011525 <- make_cnv_triptych(sample_21011525_plot[[1]],
                  sample_21011525_plot[[2]],
                  sample_21011525_plot[[3]],
                  sample_21011525_plot[[4]],
                  sample_21011525_plot[[5]])
...
egfr_plot <- make_labno_plot(df = all_patient_calls |> 
                               filter(labno != "23038464"),
                             gene = "EGFR", interval = 200000)

egfr_triptych <- make_cnv_triptych(egfr_plot[[1]],
                  egfr_plot[[2]],
                  egfr_plot[[3]],
                  egfr_plot[[4]],
                  egfr_plot[[5]])
...
egfr_triptych_fc <- make_cnv_triptych(egfr_fc_plot[[1]],
                  egfr_fc_plot[[2]],
                  egfr_fc_plot[[3]],
                  egfr_fc_plot[[4]],
                  egfr_fc_plot[[5]])
...
egfr_fc_plot_zoom <- make_fold_change_plot(
  df = all_patient_calls |> 
    filter(cnv_region_length < 500000),
  ymax = 70, 
  gene = "EGFR",
  interval = 50000)
  
egfr_triptych_fc_zoom <- make_cnv_triptych(egfr_fc_plot_zoom[[1]],
                  egfr_fc_plot_zoom[[2]],
                  egfr_fc_plot_zoom[[3]],
                  egfr_fc_plot_zoom[[4]],
                  egfr_fc_plot_zoom[[5]])
...
braf_triptych <- make_cnv_triptych(braf_plot[[1]],
                  braf_plot[[2]],
                  braf_plot[[3]],
                  braf_plot[[4]],
                  braf_plot[[5]])
...
met_triptych <- make_cnv_triptych(met_plot[[1]],
                  met_plot[[2]],
                  met_plot[[3]],
                  met_plot[[4]],
                  met_plot[[5]])
...
myc_triptych <- make_cnv_triptych(myc_plot[[1]],
                  myc_plot[[2]],
                  myc_plot[[3]],
                  myc_plot[[4]],
                  myc_plot[[5]])
...
x |> 
  ...
  filter(name %in% grep(pattern = "ERBB2", ignore.case = TRUE,
     x = unique(x$name), value = TRUE)) |>  
  filter(!duplicated(labno_suffix_worksheet)) |> view ()



  filter(duplicated(labno, fromLast = TRUE) | duplicated(labno, fromLast = FALSE)) |> 
  ggplot(aes(x = labno, 
             y = regional_fold_change)) +
  geom_jitter(pch = 21) +
  theme_bw() +

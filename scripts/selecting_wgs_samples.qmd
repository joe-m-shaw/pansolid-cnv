...
labnos_run_on_pansolid <- c("23023889", "23024556", "23025733", "23026481",
                            "23011114", "23013975", "23024575", "23032153",
                            "23033921", "24017319", "24017321")

labnos_for_brain_matrix_samples <- brain_matrix_lab_ids$mol_db_number

pansolid_samples_extractions <- extraction_tbl |> 
  filter(lab_no %in% labnos_for_brain_matrix_samples) |> 
  collect()
...
pansolid_samples_identifiers <- sample_tbl |> 
  filter(labno %in% labnos_for_brain_matrix_samples) |> 
...
salford_addresses <- unique(grep(x = pansolid_samples_identifiers$consultant_address_check,
                                 pattern = "salford", ignore.case = TRUE,
                                 value = TRUE))

cnv_validation_ffpe_information <- pansolid_samples_identifiers |> 
  ...
  filter(extraction_type == "Fresh tissue" &
           consultant_address_check %in% salford_addresses) |> 
  select(-c(consultant_address, macro, extraction_type))

csv_timestamp(cnv_validation_ffpe_information)

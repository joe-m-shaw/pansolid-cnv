...
Then I can pull DNA concentration information from DNA Database.
...
ps_sample_info <- sample_tbl |> 
  ...
  select(labno, i_gene_r_no, concentration, tissue) |> 
  collect() |> 
...
I can use the "check 1" comments to identify pathogenic variants.
...
sample_selection <- read_csv(paste0(config::get("data_folderpath"),
...
glasgow_samples_export_full <- glasgow_samples_good_quality |> 
  filter(!duplicated(labno)) |> 
  select(type, i_gene_r_no, labno, worksheet, tissue_type,
...
export_folder <- paste0(config::get("data_folderpath"),
                        "live_service/samples_for_glasgow/")

write_csv(glasgow_samples_export_full,
          file = paste0(export_folder,

...
write_csv(ps_filepath_df_annotated,
          paste0(glasgow_sample_folder,
                 "ps_filepath_df_annotated.csv"))
...
ps_filepath_df_annotated <- read_csv(paste0(glasgow_sample_folder,
                                     "ps_filepath_df_annotated.csv"),
                                     col_types = "cc")
...
For context, the PanSolid NGS workflow requires \~100ng of DNA.
...
glasgow_sample_dna_volumes <- glasgow_samples_export_filtered |> 
  ...
  remaining_dna_ng = remaining_volume * stock_concentration,
...
write_csv(glasgow_sample_dna_volumes |> 
   select(labno, firstname, surname,
          stock_volume, stock_concentration,
           send_dna_volume, send_water_volume,
           send_volume,
           send_concentration),
   file = paste0(glasgow_sample_folder,
                 "glasgow_sample_dna_volumes.csv"))
...
Here is some data for Ben McAllister to practice on.

```{r}

data_for_ben <- ps_df_join |> 
  rename(id = labno,
         tumour_type = type,
         dna_conc = concentration,
         quality1 = percent_138x,
         quality2 = stdev_noise) |> 
  select(id, tumour_type, dna_conc,
         tissue_type, quality1, quality2)

write_csv(data_for_ben,
          paste0(glasgow_sample_folder,
                 "data_for_ben.csv"))
...
# Some plots you can draw with the data

ggplot(data_for_ben, aes(x = tumour_type,
                         y = dna_conc)) +
  geom_boxplot()

ggplot(data_for_ben, aes(x = quality1,
                         y = quality2)) +
  geom_point(shape = 21) 
...
ggplot(data_for_ben, aes(x = tissue_type,
                         y = dna_conc)) +
  geom_boxplot()

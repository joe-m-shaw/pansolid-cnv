...
format:
  html:
    ...
    code-links:
      - text: Code link
        icon: file-code
        href: https://github.com/joe-m-shaw/pansolid-cnv/blob/main/scripts/dev/chris_tert_promoter_variants.qmd
editor: visual
---
...
labno_df <- sample_tbl |>
  ...
  select(labno, i_gene_r_no, concentration) |>
  collect() |> 
...
tert_canonical_variant_df_conc <- tert_canonical_variant_df |> 
  ...
  select(labno, i_gene_r_no, 
         worksheet, variant_nomenclature, dna_concentration) |> 
...
no_tert_variant_df_mod <- no_tert_variant_df |> 
  ...
  left_join(labno_df, by = "labno",
            relationship = "many-to-one") |> 
   select(labno, i_gene_r_no, worksheet, dna_concentration) |> 

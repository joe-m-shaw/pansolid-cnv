...
24031557 and 24044746: in each sample the pattern seems to be a mono-allelic 
deletion, which is in agreement with the *NF2* PCR results.
...
If we assume that a scientist inspecting the SNP track would spot the calls
...
The PanSolid CLC pipeline has good concordance for LOH analysis with *NF2*
dinucleotide marker PCR analysis.

When quality thresholds are applied (noise <= 0.7 and percent coverage at 
138X >= 75%), there are only 3 discrepant results.

The discrepant samples would be classified as deletions based

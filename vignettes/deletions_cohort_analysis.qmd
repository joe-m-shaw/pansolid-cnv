...
- PanSolid can miss cases where chromosomes have a copy number of 1 (examples: 24025266, 24025266, 24021613), especially when the NCC is low (examples: 24018922, 24017321)

- Non-diploid samples are tricky. PanSolid detects loss from the tumour ploidy, not absolute copy number. This means PanSolid may flag a deletion, but the gene actually has a copy number of 2 and the rest of the genome is at a higher ploidy (examples: 23033921, 24030951, 24042543). This can also happen with disordered diploid genomes (example: 24035920).

- Signal-adjusted noise above 1 leads to false positive deletion calls on both coarse and fine settings (examples: 23011114, 24030364)

- CDKN2A is detected the same by coarse and fine in most cases, but in one case coarse did not detection and deletion detected on fine (24048159).

- Fine is the better choice for PTEN, although it does sometimes detect very small intragenic PTEN deletions not seen on WGS (24042543).

- Fine gives small intragenic deletions in PMS2 in multiple samples (examples: 24031053)
...
A CDKN2A deletion is detected on fine but not coarse. This deletion is seen on WGS.

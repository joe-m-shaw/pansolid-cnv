...
![*MYC* amplification with a chromosome 8q gain in sample 24024116. This demonstrates the pipeline's ability to detect focal amplifications against a background of raised copy number. ](`r paste0(data_folder, "images/", "sample_24024116_whole_genome.png")`){#fig-sample-24024116-whole-genome}
...
Over-estimation of both *EGFR* and *ERBB2* for NGS and ddPCR, which was previously noted for the ERBB2 validation. However there was close agreement despite this bias. This was for pure 100% FFPE mixed samples, so NCC wasn't an issue.
However for normal samples, NCC is a problem.
The sample is sequenced in bulk, without single-cell resolution.
To derive the number of gene copies per tumour cell requires you to calculate this value based on the fold change value and the NCC value.
This also assumes that:
1 - the NCC value is entirely accurate to the area of sample from which DNA was extracted
2 - the cancer cells all have the same number of gene copies, whereas in reality this can vary.

The biggest variable to quantification will be the NCC value provided.

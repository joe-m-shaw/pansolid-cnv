...
```{r}
#| label: tbl-30-percent-fold-change-values
#| tbl-cap: 
#| echo: FALSE
...
fold_change_plot <- fold_change_df |> 
  ...
  geom_point(data = point_data, shape = 21, fill = "white", size = 2) +
  theme_bw() +
...
```{r}
#| label: tbl-fold-change-predicted-lod
#| echo: FALSE

theoretical_lod_df <- point_data |> 
  arrange(desc(ncc)) |> 
  mutate(theoretical_lod_target_copies = round(target_copies, 0)) 

measured_lod_df <- data.frame(
  ncc = c(100, 30),
  measured_lod_target_copies = 
    c("5 for EGFR, ERBB2, MET and MYCN; 8 for MYC and BRAF",
      "14 for BRAF, EGFR, ERBB2, MET and MYCN")
)

lod_df_joined <- theoretical_lod_df |> 
  left_join(measured_lod_df, by = "ncc") |> 
  select(-target_copies) |> 
  rename(
    "Neoplastic cell content (%)" = ncc,
    "Predicted limit of detection in gene copies per cell (rounded) with a fold change threshold of 3" = theoretical_lod_target_copies,
    "Measured limit of detection in gene copies per cell" = measured_lod_target_copies
  )
...
Two approaches were used to determine the limit of detection: undiluted SeraCare reference materials with known quantities of target genes added, and mixtures of these controls to simulate clinical samples with different NCC values. All genes were detected in the +6 copies SeraCare control, and all genes except *MYC* were detected in the +12 copies control when 30% NCC was simulated.

...
**MYCN**: avian MYeloCytomatosis oncogene Neuroblastoma sub-type
...
- ***MDM2***: *MDM2* amplifications are linked to tyrosine-kinase inihibitor resistance in multiple cancers [@hou2019] and targeted anti-MDM2 treatments are in development [@sciot2021].
...
- ***MYCN***: in neuroblastoma, MYCN amplification is associated with advanced disease and is used for risk stratification []
...
results_by_cancer_type <- ggplot(amp_results_with_cancer_types |> 
  ...
  labs(x = "", y = "Fold change",
       title = "PanSolid results by cancer type",
       caption = "Note: results below fold change of 1 are changed to 1 to allow log scale plotting",
       fill = "")
...
```{r}
#| label: egfr-gene-coordinate-plots
#| echo: FALSE
...
```{r}
#| label: met-gene-coordinate-plots
#| echo: FALSE
...
```{r}
#| label: braf-gene-coordinate-plots
#| echo: FALSE

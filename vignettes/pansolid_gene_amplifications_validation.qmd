...
intra_run_summary <- repeat_data_fold_change_range |> 
  filter(labno_suffix_worksheet %in% intra_run_replicates) |> 
    group_by(labno, gene) |> 
    summarise(sd = sd( max_region_fold_change ),
              max = max( max_region_fold_change ),
              min = min( max_region_fold_change ),
              range = max - min,
              n = n(),
              z = (n-1)*sd^2)
...
all_repeats_pooled_sd <- all_repeats_run_summary |> 
  group_by(gene) |> 
  summarise(pooled_sd = round(sqrt(sum(z) / 
                            (sum(n))), 2))
...
The intra-run pooled standard deviation in the fold change range of -2 to 10 was `r intra_pooled_sd`, and the inter-run pooled standard deviation for the same fold change range was `r inter_pooled_sd`. 
...
repeat_plot_all_genes <- ggplot(repeat_all_amp_data, aes(x = gene, y = max_region_fold_change)) +
...
repeat_plot_24027774_myc <- ggplot(repeat_all_amp_data |> 
                                 filter(labno %in% c("24027774") &
                                          gene %in% c("MYC")), aes(x = gene, y = max_region_fold_change)) +
  geom_jitter(shape = 21, width = 0.2) +
  theme_bw() +
  geom_hline(yintercept = 2.8, linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 90),
...
```{r}
#| label: repeatability-plots-with-max-and-min
#| include: FALSE
...
repeat_all_amp_data_long <- repeat_all_amp_data |> 
  ...
  mutate(gene_labno_suffix_worksheet = str_c(gene, "_", labno_suffix_worksheet)) 

repeat_plot_24017319 <- repeat_all_amp_data_long |> 
  filter(labno == "24017319") |>
  ggplot(aes(x = gene_labno_suffix_worksheet, y = fold_change)) +
    geom_line() +
    geom_point(shape = 21, aes(fill = gene)) +
    theme_bw() +
    theme(axis.text.x = element_blank()) +
    labs(x = "", y = "Fold change") +
    coord_cartesian(ylim = c(-2, 8)) +
    scale_y_continuous(breaks = c(seq(-2,8, by = 2))) +
    geom_hline(yintercept = 2.8, colour = "red") +

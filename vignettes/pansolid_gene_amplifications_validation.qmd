...
When the larger genes were analysed on the "fine" graining level, we frequently observed multiple intra-genic CNV regions with slightly different fold changes. These multiple intra-genic CNV calls were observed in SeraCare reference materials despite the materials containing equal spike-in quantities of each exon, and hence we concluded that the additional calls were the result of sample noise rather than genuine CNV breakpoints. Because of this, we decided to analyse the larger genes on the "coarse" graining level, rather than "fine".
...
pansolid_vs_wgs_summary <- pansolid_vs_wgs_amps_selection_cnvs_above_ten |> 
...
#| tbl-cap: "PanSolid NGS test metrics versus WGS"
#| echo: FALSE
...
There was a single apparent false negative result: for sample 24030962, which was extracted using the QIAsymphony method, PanSolid NGS did not detect an amplification for *EGFR*, whilst DNA extracted from fresh frozen tissue form the sample pathology block (23033921) had an *EGFR* amplification of 14 copies detected on WGS along with a chromosome 7 copy number of 6. The same DNA from fresh-frozen tissue showed a borderline *EGFR* fold change of 2.7 when tested using PanSolid NGS.
...
adjusted_wgs_pansolid_metrics <-get_epir_metrics(outcome_vector = c(
  sum(pansolid_vs_wgs_summary$`True positives`),
  sum(pansolid_vs_wgs_summary$`False positives`),
  sum(pansolid_vs_wgs_summary$`False negatives`) - 1,
  sum(pansolid_vs_wgs_summary$`True negatives`)
))
...
diploid_wgs <- wgs_html_ids |> 
  filter(nhsno %in% pansolid_vs_wgs_amps_selection$nhsno) |> 
  filter(calculated_overall_ploidy < 4)

diploid_max <- max(diploid_wgs$calculated_overall_ploidy)

diploid_min <- min(diploid_wgs$calculated_overall_ploidy)

tetraploid_wgs <- wgs_html_ids |> 
  filter(nhsno %in% pansolid_vs_wgs_amps_selection$nhsno) |> 
  filter(calculated_overall_ploidy > 4)

```

In `r nrow(diploid_wgs)` WGS samples the tumours appeared to be diploid, with ploidy estimates from the Canvas software ranging from `r diploid_min` to `r diploid_max`. However, `r nrow(tetraploid_wgs)` samples the ploidy was approximately 4 (fresh frozen DNA extractions: 23018367 and 23033921, QIAsymphony DNA extractions: 24030951 and 24030962), indicating tetraploid genomes. The results for one sample are shown in @fig-tetraploid-genome.

![Comparison of results from WGS and PanSolid NGS for a tetraploid tumour sample. The WGS result (top) identified the base chromosome count as 4, with 90% tumour cell content. Convsersely, PanSolid NGS does not currently detect different ploidy states.](`r paste0(data_folder, "images/", "tetraploid_genome_comparison.png")`){#fig-tetraploid-genome}
...
#| fig-cap: "The results of repeat replicates for sample 24017319. Both maximum and minimum fold changes for each gene are plotted, and are connected by a black line if they are different values. The solid line shows the fold change threshold of 2.8"
#| fig-height: 6
#| fig-width: 6
#| echo: FALSE
...
### Fold change uncertainty of measurement
...
intra_run_summary <- repeat_data_fold_change_range |> 
  filter(labno_suffix_worksheet %in% intra_run_replicates) |> 
    group_by(labno, gene) |> 
    summarise(sd = sd( fold_change_for_uom ),
              n = n(),
              n_1 = n-1,
              z = (n-1)*sd^2)
  
intra_pooled_sd <- round(sqrt(sum(intra_run_summary$z) / 
                            (sum(intra_run_summary$n_1))), 3)
...
inter_pooled_sd <- round(sqrt(sum(inter_run_summary$z) / 
                            (sum(inter_run_summary$n_1))), 3)


all_repeats_run_summary <- repeat_data_fold_change_range |> 
    group_by(labno, gene) |> 
    summarise(sd = sd( fold_change_for_uom ),
              n = n(),
              max = max(fold_change_for_uom),
              min = min(fold_change_for_uom),
              range = max - min,
              n_1 = n-1,
              z = (n-1)*sd^2) |> 
  filter(!is.na(sd))
  
all_repeats_pooled_sd <- round(sqrt(sum(all_repeats_run_summary$z) / 
                            (sum(all_repeats_run_summary$n_1))), 3)
...
fold_change_uom_upper <- fold_change_threshold + (2 * inter_pooled_sd)

fold_change_uom_lower <- fold_change_threshold - (2 * inter_pooled_sd)

validation_results_with_uom <- amp_validation_all_amp_gene_results_collated |> 
  mutate(uom_category = case_when(
    max_region_fold_change > fold_change_uom_upper ~"Amplified",
    max_region_fold_change < fold_change_uom_lower ~"Not amplified",
    max_region_fold_change <= fold_change_uom_upper &
      max_region_fold_change >= fold_change_uom_lower ~"Borderline"))
...
This method calculated the intra-run pooled standard deviation in the fold change range of -2 to 10 as `r intra_pooled_sd`, and the inter-run pooled standard deviation for the same fold change range as `r inter_pooled_sd`. The inter-run standard deviation was chosen as the standard deviation value for the uncertainty of measurement, as it is the most conservative.

Taking the assumption that repeat variation in the measurement of fold change follows a normal distribution, 95% of repeat values would be expected to fall within ± 2 standard deviations of the mean. Consequently, the uncertainty of measurement for a fold change value in the range of -1 to 10 can be taken as ± `r (2*inter_pooled_sd)` (± 2 x `r inter_pooled_sd`) at a confidence level of 95%. The uncertainty of measurement was used create a "grey zone" of 2 standard deviations around the fold change threshold (@fig-fold-change-uom-plot). Out of `r nrow(validation_results_with_uom)` results, only `r nrow(validation_results_with_uom[validation_results_with_uom$uom_category == "Borderline",])` fell within this borderline range.

```{r}
#| label: fig-fold-change-uom-plot
#| fig-cap: "All the fold change results for PanSolid NGS validation samples, coloured by their relation to the fold change threshold with uncertainty of measurement. The solid black line shows the fold change threshold of 2.8, and the dashed lines show the fold change threshold ± 2 standard deviations. The Y axis is -2 to 4 to make the plot clearer - amplifications with fold changes above 4 are not shown."
#| fig-width: 6
#| echo: FALSE

fold_change_uom_plot <- ggplot(validation_results_with_uom, aes(x = gene,
                                                         y = max_region_fold_change)) +
  geom_hline(yintercept = fold_change_threshold) +
  geom_hline(yintercept = fold_change_uom_upper, linetype = "dashed") +
  geom_hline(yintercept = fold_change_uom_lower, linetype = "dashed") +
  geom_jitter(shape = 21, aes(fill = uom_category), width = 0.2, alpha = 0.6) +
  scale_fill_manual(values = c("#CC6677", "#888888", "#88CCEE")) +
  theme_bw() +
  theme(legend.position = "right",
        panel.grid = element_blank()) +
  coord_cartesian(ylim = c(-2, 4)) +
  scale_y_continuous(breaks = c(-2, 0, 2, 2.8, 4)) +
...
fold_change_uom_plot_zoomed_out <- ggplot(validation_results_with_uom, aes(x = gene,
                                                         y = max_region_fold_change)) +
  geom_hline(yintercept = fold_change_threshold) +
  geom_hline(yintercept = fold_change_uom_upper, linetype = "dashed") +
  geom_hline(yintercept = fold_change_uom_lower, linetype = "dashed") +
...
ddpcr_comparison_plot <- ggplot(seracare_ddpcr_comparison, 
  ...
  labs(x = "Total copies ddPCR (Manchester)",
       ...
       title = "North West GLH ddPCR vs SeraCare ddPCR",
       fill = "")
...
discordant results, 4 of which were borderline by both methods; the remaining result was likely due to a copy number change at the locus of the ddPCR reference gene, *AP3B1*. Overall, PanSolid NGS and ddPCR agreed on 
...
As samples are sequenced in bulk, PanSolid NGS provides a single "fold change" value of sequencing coverage relative to a control cohort at a gene locus, rather than quantifying the exact number of gene copies present within individual cells. The number of gene copies per tumour cell can be estimated based on the fold change value and the NCC value provided for the sample. However this calculation assumes that the NCC value is entirely accurate to the area of sample from which DNA was extracted, and that each cancer cell has the same number of gene copies. These difficulties are avoided when analysing synthetic reference materials, where additional genetic material has been added and hence the "NCC" can be considered 100%.
...
The copy number of a gene is 

Add description of copy number thresholds with reference to literature and whether copy number is in relation to the chromosome or ploidy level.

In the validation cohort, 2 glioblastoma samples had ploidy levels of 4 calculated by WGS.
@boisselier2018 showed that 11% of glioblastoma cases have whole genome duplication causing tetraploidy.
...
- Tetraploid genomes can impact copy number calculations.
...
when compared to ddPCR (@sec-results-ddpcr-comparison), and a sensitivity and specificity of 
`r wgs_pansolid_sensitivity`%
and
`r wgs_pansolid_specificity`%
when compared to WGS (@sec-results-wgs-comparison). One WGS sample had a discordant result that appeared to be due to intra-tumour heterogeneity (@sec-wgs-discordant-result). If this apparent false negative is removed, the sensitivity of PanSolid NGS in comparison to WGS increases to
`r adjusted_wgs_pansolid_sensitivity`%.
...
- Analysts should take the uncertainty of measurement into account.

- The method cannot distinguish between ploidy states.

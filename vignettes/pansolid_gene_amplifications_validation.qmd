...
Gene amplification is a common mechanism in the development of cancer [@matsui2013]. Additional gene copies can be present on extra-chromosomal DNA rather than their initial chromosomal loci [@verhaak2019], for example *EGFR* amplifications can be present on extrachromosomal DNA fragments referred to as “double minutes” [@french2019]. “Double minute” refers to a very small (“minute”) bodies of chromatin material with two arms (“double) which were originally visualised by chromosome staining [@cox1965].
...
The PanSolid enrichment includes 212 genes relevant to cancer. Of these, there are 9 genes which were selected as having amplifications with clinical relevance (@tbl-amplification-coordinates).
...
In total, 94 different samples were tested on the PanSolid version 2 enrichment during this validation. 
...
`r round(min(validation_sample_patient_info$years_at_sample_receipt, na.rm = TRUE), 0)`
 to 
`r round(max(validation_sample_patient_info$years_at_sample_receipt, na.rm = TRUE), 0)`
years (median: `r round(median(validation_sample_patient_info$years_at_sample_receipt, na.rm = TRUE), 0)` years).
...
The details of the SeraCare controls are shown in @tbl-seracare-reference-material-details. Note that there were two tubes of both the Seraseq® TNA (DNA) WT Mix and the Seraseq® Solid Tumor CNV Mix, +12 copies mix.
...
Validation analysis was performed using the R programming language (version 4.4.1) [@rcore2024] relying on packages from the tidyverse [@wickham2019]. This validation document is written in Quarto [@Allaire_Quarto_2024] and is publicly available on [Github](https://github.com/joeshaw824/pansolid_cnv_validation).
...
good_quality_samples <- validation_stdev_results_collated |> 
  filter(st_dev_signal_adjusted_log2_ratios < 0.7)

pos_results_for_plot <- validation_pos_cnv_results_collated |> 
  filter(labno %in% good_quality_samples$labno)

```

Overall there are PanSolid NGS results for `r nrow(validation_stdev_results_collated)` replicates representing `r length(unique(validation_stdev_results_collated$labno))` different samples.
Of these, `r nrow(good_quality_samples)` had signal-adjusted noise metrics within the acceptable range of 0 - 0.7.
There was a range of different amplifications detected across different cancer types.

```{r}
#| label: pansolid-results-by-cancer-type
#| include: FALSE

amp_results_with_cancer_types <- validation_all_amp_gene_results_collated |> 
  left_join(validation_sample_patient_info,
            by = "labno", 
            relationship = "many-to-one")

results_by_cancer_type <- ggplot(amp_results_with_cancer_types |> 
         filter(!cancer_type_new %in% c("Error", "Reference material",
                                        NA)) |> 
         mutate(cancer_type_new = case_when(
           cancer_type_new == "central nervous system" ~"CNS",
           cancer_type_new == "hepatocellular carcinoma" ~"liver",
           TRUE ~cancer_type_new
         ),
  ...
  facet_wrap(~cancer_type_new, ncol = 4) +
...
#| fig-subcap: "Gene amplification results from PanSolid NGS arranged by cancer type. Results below a fold change of 1 are changed to 1 to allow log scale plotting. CNS: central nervous system."
#| fig-height: 6
#| fig-width: 6
#| echo: FALSE
...
## Coarse setting: *ALK*, *BRAF*, *EGFR*, *ERBB2*, *MET*, *MDM2* and *SMO* results

```{r}
#| label: gene-coordinate-plots
#| echo: FALSE
...
```{r}
#| label: fig-egfr-plot
#| fig-cap: "Amplification calls within the EGFR gene."
#| echo: FALSE
...
```{r}
#| label: fig-met-plot
#| fig-cap: "Amplification calls within the MET gene."
#| echo: FALSE
...
```{r}
#| label: fig-braf-plot
#| fig-cap: "Amplification calls within the BRAF gene."
#| echo: FALSE
...
## Fine setting: *MYC* and *MYCN* results

```{r}
#| label: myc-amplifications
#| echo: FALSE

myc_samples <- pos_results_for_plot |> 
  filter(gene == "MYC")

```

*MYC* amplifications were detected in `r length(unique(myc_samples$labno))` samples, with fold changes ranging from `r round(min(myc_samples$fold_change), 1)` to `r round(max(myc_samples$fold_change), 1)`.
The full width of the *MYC* gene was covered in all calls except for sample 24031627, in which only the latter 2 exons of the gene had an amplification.
...
myc_plot_narrow <- make_cnv_triptych_plot(input_plot = make_labno_cnv_plot(
  df = pos_results_for_plot |> 
...
myc_plot_wide_zoom <- myc_labno_plot[[4]] +
...
```{r}
#| label: cnv-length-distribution
#| echo: FALSE

cnv_length_plot <- pos_results_for_plot |> 
  filter(gene != "no positive calls") |> 
  mutate(cnv_length_kb = cnv_length / 1000) |> 
  ggplot(aes(x = cnv_length_kb, y = )) +
...
pansolid_wgs_sample_selection <- pansolid_wgs_samples_qs |> 
...
The results from the PanSolid pipeline compared to the results from WGS are shown in @tbl-wgs-comparison. These results are only for comparisons where the WGS and PanSolid DNA samples were extracted from the same pathology block, and where the PanSolid sample was extracted via the QIAsymphony method. Gains detected on WGS with fewer than 10 gene copies are coded as normal results.
...
egfr_wgs_pansolid_dosage_plot <- wgs_egfr_calls |> 
  ...
  ggplot(aes(x = cnv_copy_number, y = pansolid_copy_number)) +
...
Intra-run variation (repeatability) and inter-run variation (reproducibility) were investigated for 3 samples using the testing structure shown in 
...
The variation in the measured fold change between replicates was minimal, with no variation across the fold change threshold of 2.8.
...
poor_quality_samples <- validation_stdev_results_collated |> 
  filter(st_dev_signal_adjusted_log2_ratios >= 0.7)

```

Overall, the amount of signal-adjusted noise within the cohort ranged from 
`r round(min(validation_stdev_results_collated$st_dev_signal_adjusted_log2_ratios), 1)`
-
`r round(max(validation_stdev_results_collated$st_dev_signal_adjusted_log2_ratios), 1)`
 with a median of 
 `r round(median(validation_stdev_results_collated$st_dev_signal_adjusted_log2_ratios), 1)`
 .
There were 
...
All these samples were extracted using the Cobas method.

```{r}
#| label: quality-control
#| echo: FALSE

overall_noise_plot <- ggplot(validation_stdev_results_collated, aes(x = st_dev_signal_adjusted_log2_ratios,
                                              y = )) +
...
poor_quality_results <- validation_pos_cnv_results_collated |> 
...
```{r}
#| label: overall-sensitivities
#| echo: FALSE

ddpcr_confirmation_summary <- ddpcr_vs_pansolid |> 
  filter(!duplicated(labno_suffix_gene) & !sample %in% seracare_ids$labno) |> 
  count(gene, outcome) |> 
  pivot_wider(id_cols = gene, names_from = outcome, values_from = n,
              values_fill = 0) |> 
  mutate(ddpcr_positive = true_positive + false_negative,
         ddpcr_negative = true_negative) |> 
  select(gene, ddpcr_positive, ddpcr_negative)

wgs_confirmation_summary <- wgs_pansolid_comparison |> 
  mutate(labno_gene = str_c(labno, "_", gene)) |> 
  filter(!duplicated(labno_gene) & !labno %in% seracare_ids$labno) |> 
  count(gene, outcome) |> 
  pivot_wider(id_cols = gene, names_from = outcome, values_from = n,
              values_fill = 0) |> 
  mutate(wgs_positive = true_positive + false_negative,
         wgs_negative = true_negative) |> 
  select(gene, wgs_positive, wgs_negative)

seracare_confirmation_summary <- seracare_gene_overlap |> 
  janitor::clean_names() |> 
  filter(targetted_within_this_validation == "Yes") |> 
  mutate(seracare_positive = 3,
         seracare_negative = 1)
  
confirmation_summary <- wgs_confirmation_summary |> 
  left_join(ddpcr_confirmation_summary, by = "gene") |> 
  left_join(seracare_confirmation_summary |> 
              select(-targetted_within_this_validation), by = "gene")

confirmation_summary[is.na(confirmation_summary)] <- 0

confirmation_summary_mod <- confirmation_summary |> 
  mutate(total_positive = wgs_positive + ddpcr_positive + seracare_positive,
         total_negative = wgs_negative + ddpcr_negative + seracare_negative)

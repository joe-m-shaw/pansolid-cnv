...
seracare_pansolid_results_plot <- ggplot(seracare_ngs_comparison, 
  ...
  facet_wrap(~gene, ncol = 2) +
  labs(x = "Total copies (additional copies)", y = "PanSolid fold change", title = "SeraCare results by target gene") +
...
Dilution mixes were prepared to simulate a sample with neoplastic cell contents of 30%, 20%, 10% and 0%, and were tested on the PanSolid v2 enrichment on worksheet WS144291. All genes except *MYC* were detected at a simulated NCC of 30%, whilst below this level no genes exceeded the fold change threshold of 2.8 (@fig-seracare-dilution-mix-results), although indications of the amplifications could were evident on the whole genome plots (@fig-seracare-dilution-mixes-png). 

Notably, a fold change threshold of 2.8 is predicted to have a limit of detection composed of many different combinations of NCC values and target copies per cell. This limit of detection is predicted to include samples with an NCC of 30% and 14 gene copies per tumour cell (12 additional copies), and this is concordant with the results from the SeraCare dilution mix experiment (@fig-fold-change-plot).
...
```{r}
#| label: fig-fold-change-plot
#| fig-cap: "The plot shows the relationship between neoplastic cell content and the number of target gene copies per cell, with the black curve showing the predicted detection limit with a fold change of 2.8. The predicted position of a sample with an NCC of 30% and 14 copies per tumour cell is shown as a white dot, which sits on the limit of detection."
#| echo: FALSE
...
fold_change_plot <- fold_change_df |> 
  ...
  scale_x_continuous(limits = c(0, 100), breaks = seq(0,100,10),
                     minor_breaks = FALSE) +
  annotate(geom = "text", x = 30, y = 30, size = 4, label = "Detected") +
  annotate(geom = "text", x = 30, y = 5, size = 4, label = "Not detected") + 
  annotate(geom = "segment", x = 0, xend = 14, y = 30, yend = 30, linetype = "dashed") +
  annotate(geom = "segment", x = 14, xend = 14, y = 0, yend = 30, linetype = "dashed") +
  annotate(geom = "point", x = 14, y = 30, shape = 21, fill = "white", size = 2)
...
- The CNV regions identified by the test may be narrower than the true region of underlying varition: this is a known limitation of performing CNV detection using an NGS enrichment method (@sec-wgs-coordinate-correlation). 

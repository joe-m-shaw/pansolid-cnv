...
The files in the "clc_processed_outputs" folder are all the processed files generated for samples extracted via cobas® or QIAsymphony extractions, and tested on PanSolid version 1 or version 2.
These files are structured in the agreed format we finally decided on for analysis, with the tables and plots described in the SOP (DOC6261).
...
The sample information from the DNA Database was extracted using the erbb2_val_dna_db_queries.R script which uses an ODBC connection.
The information was exported as csv files so that this Quarto document should render without the user needing an ODBC connection to the DNA Database.
...
Neoplastic cell content can be parsed from the "comments" section of DNA Database, but often it is missing so I've had to manually created a csv file with the missing information.
I got this information by checking the scanned referral forms on DNA Database, which usually have the neoplastic cell content written on.
...
Initially I attempted to use the DISCODE field in DNA Database to identify the different tumour sources but this wasn't ideal (often multiple tumour sources map to the same disease code).
Then I manually checked the scanned referral forms for each sample on DNA Database and collated the results in the cnv_validation_labnos_and_tissue_sources.csv file.
...
The DNA concentrations I've pulled from the submission sheets for the PanSolid workflow.
This works for most samples except for 3 which for some reason weren't in the standard PanSolid Excels.
...
NHS numbers can be pulled off DLMS although annoyingly for two lab numbers they aren't included (21015264 and 24008372).
The NHS numbers are on different samples for the same patients, so I've added the NHS numbers to an additional csv file and added them in below.
This avoids hard-coding patient-identifiable information into this Quarto.
...
Adding on the DNA Database information to each table of results allows these tables to act as the base for downstream analysis.
The idea is that each table has all the variables I might want to filter by (DNA extraction method, NHS number, PanSolid version, DNA concentration etc) and so reduces joining with other tables downstream.
...
The pooled standard deviation of the ERBB2 fold change in `r length(unique(repeat_results$labno_suffix_worksheet))` repeat replicates of `r length(unique(repeat_results$labno))` DNA samples was `r erb2_fold_change_sd[[2]]`.
The pooled standard deviation of the signal-adjusted noise for the same cohort was `r signal_noise_sd[[2]]`.
...
```{r}
#| label: erbb2-monitoring-data
#| include: FALSE

live_service_amp_genes <- read_csv(paste0(config::get("data_folderpath"),
                                          "live_service/",
                                          "collated/",
...
# Load panel information separately (for samples from 2024, panel information
# was not included in filename)
live_service_panels <- read_csv(paste0(config::get("data_folderpath"),
                                          "live_service/",
                                          "collated/",
                                          "pansolid_amplifications_live_service/",
                                       "pansolidv2_sample_worksheet_panel_information.csv"))

live_service_stdev <- read_csv(paste0(config::get("data_folderpath"),
                                          "live_service/",
                                          "collated/",
                                          "pansolid_amplifications_live_service/",
...
```{r}
#| label: erbb2-monitoring-join-data
#| include: FALSE

crc_panels <- live_service_panels |> 
  filter(grepl(pattern = "CRC", 
               x = panel))

amp_genes_crc <- live_service_amp_genes |> 
  filter(gene == "ERBB2") |> 
  filter(!duplicated(labno)) |> 
  # Remove worksheet as collated data contains some NA values, due to the
  # filename structure changing during the monitoring period
  select(-worksheet) |> 
  inner_join(crc_panels |> 
              select(filename, worksheet, panel),
            by = "filename",
            relationship = "one-to-one") |> 
  left_join(live_service_stdev |> 
              select(filename, st_dev_signal_adjusted_log2_ratios),
            by = "filename",
            relationship = "one-to-one") |> 
  mutate(quality_category = case_when(
    st_dev_signal_adjusted_log2_ratios >= 1 ~"fail",
    st_dev_signal_adjusted_log2_ratios < 1 ~"pass"
  ),
  erbb2_category = case_when(
...
stopifnot(anyNA(amp_genes_crc$worksheet_number) == FALSE)

stopifnot(anyNA(amp_genes_crc$erbb2_category) == FALSE)

stopifnot(anyNA(amp_genes_crc$quality_category) == FALSE)

```

Data were reviewed for `r length(unique(amp_genes_crc$labno))` samples tested on
`r length(unique(amp_genes_crc$worksheet))` worksheets between April 2024
(worksheet WS`r as.character(min(amp_genes_crc$worksheet_number))`) 
and April 2025 (worksheet WS`r as.character(max(amp_genes_crc$worksheet_number))`).

```{r}
#| label: erbb2-monitoring-quality
#| echo: FALSE

erbb2_monitoring_quality <- amp_genes_crc |> 
  group_by(quality_category) |> 
  summarise(total = n()) |> 
  mutate(percent = round((total/sum(total))*100, 1)) |> 
  janitor::adorn_totals()

knitr::kable(erbb2_monitoring_quality)

csv_timestamp(table = erbb2_monitoring_quality, 
              folder = paste0(erbb2_data_folder,
                              "outputs/tables/"))
...
```{r}
#| label: erbb2-monitoring-amplifications
#| echo: FALSE

erbb2_monitoring_amplifications <- amp_genes_crc |> 
  filter(quality_category == "pass") |> 
  group_by(erbb2_category) |> 
  summarise(total = n()) |> 
  mutate(percent = round((total/sum(total))*100, 1)) |> 
  janitor::adorn_totals()

knitr::kable(erbb2_monitoring_amplifications)

csv_timestamp(table = erbb2_monitoring_amplifications,
              folder = paste0(erbb2_data_folder,
                              "outputs/tables/"))

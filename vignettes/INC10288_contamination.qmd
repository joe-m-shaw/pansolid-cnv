...
One way to identify contamination is to plot the SNP frequencies within a 
sample (sample 1) and then colour the data by the genotypes of the SNPs in 
the sample which 
is suspected of being the contaminant (sample 2).

If sample 1 is not contaminated with sample 2, then there should be no 
pattern to the colouring. Here is an example from the limit of detection 
experiment: sample 24026628 was analysed on it's own as "24026628a". Here are the
results coloured by a random sample from the same worksheet (25008651).
...
Now here's what the plot looks like if the SNP data is coloured by the genotypes
in sample 24053299, which was deliberately mixed into 24026628b.
...
WS146642_labnos <- all_snp_data |> 
  filter(worksheet == "WS146642")

unique(WS146642_labnos$labno)

make_snp_plot("WS146642", "24054291",
              "WS146642", "24057739")[1]

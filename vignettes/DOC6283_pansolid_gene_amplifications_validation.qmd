...
The CLC pipeline identifies CNVs according to the fold change of target regions. When the coverage in the sample is greater or equal to the coverage in the baseline normal controls, the fold change is defined in @eq-fold-change-gain, where FC = fold change and CN = copy number.
...
Because the equation used is dependent on the relative coverage of the sample, the pipeline does not produce fold change values between 1 and -1. This is described fully in the Qiagen CLC User Manual (@qiagen2023, section [Copy number and fold change](https://resources.qiagenbioinformatics.com/manuals/clcgenomicsworkbench/2305/index.php?manual=Copy_number_fold_change.html)) and shown in @fig-fold-change-equations-plot.
...
The validation analysis was performed using the R programming language (version 4.4.1) [@rcore2024] relying mainly on packages from the tidyverse [@wickham2019]. This validation document is written and rendered using Quarto [@allaire2024] and is publicly available on [Github](https://github.com/joeshaw824/pansolid_cnv_validation).
...
`r paste0(DOC6283_folder, "images/", "sample_24037724_whole_genome.png")`){#fig-sample-24037724-whole-genome}
...
- Any fold changes lower than -1, which were calculated using @eq-fold-change-loss, were converted to the output of @eq-fold-change-gain using @eq-fold-change-conversion. This is because when the coverage of the sample is less than the coverage in the control dataset, a different equation is used to calculate the fold change (@sec-methods-fold-change). Converting fold changes below -1 was performed to avoid the change in equation being misinterpreted as sample variation, which would over-estimate the uncertainty of measurement.
...
`r paste0(DOC6283_folder, "images/", "lod_seracare_controls.png")`){#fig-seracare-control-results-png}
...
discordant results, 4 of which were borderline by both methods; the remaining result was due to a copy number change at the locus of the ddPCR reference gene, *AP3B1*. When this result was removed from the analysis, PanSolid NGS and ddPCR agreed on 
`r ddpcr_pansolid_correct_proportion_updated`%
...
- The test does not detect tetraploidy when present (@sec-wgs-discordant-results), which can impact copy number calculations performed using NCC, as these calculations assume a diploid copy number. Other non-diploid karyotypes may also impact the accuracy of copy number calculations.
...
`r ddpcr_pansolid_sensitivity_updated`% 
and 
`r ddpcr_pansolid_specificity_updated`% 
when compared to ddPCR (@tbl-ddpcr-comparison-updated-metrics). When compared to WGS on diploid samples,
the sensitivity is `r adjusted_wgs_pansolid_sensitivity`% and the specificity is `r adjusted_wgs_pansolid_specificity`% (@tbl-wgs-comparison-metrics-2).

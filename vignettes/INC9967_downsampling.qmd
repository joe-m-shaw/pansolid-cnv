...
The WGS HTML reports for 24017319, 24030945 and 23032153 are saved here:

```{r}
#| label: html-filepath
#| include: TRUE

paste0(data_folder, "validation/DOC6283_amplifications/raw/wgs/")

```

The WGS HTML report for 24035924 is saved here:

```{r}
#| label: html-filepath2
#| include: TRUE

paste0(data_folder, "validation/DOC6567_deletions/raw/wgs/")
...
Setting coverage thresholds is impacted by:

-   The NCC of the sample

-   The type of CNV (amplification or deletion)

-   The degree of the change (low or high fold change)

-   The size of the change (small or large CNV)
...
-   No new false positive or false negative amplification calls were noted.

**Deletions**

-   Reducing coverage as low as 30% at 138X did not significantly impact 
whole-gene fold changes of coordinates for genes with copy numbers of 0 on 
WGS. These samples had signal-adjusted noise from 0.2 to 0.8.

-   CN1 deletions were more impacted by coverage.
...
Could we use median coverage as an alternate metric? 138X is from the SNV
validation, and doesn't seem to be useful from a CNV perspective. We have seen
samples with 0% coverage at 138X but the results look OK
(WS148333, 24068563).

# Conclusion

Thresholds of greater than 75% panel covered at 138X and signal-adjusted 
noise greater than 0.7 are a pragmatic combination for calling of 

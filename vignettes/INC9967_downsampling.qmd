...
A clear quality threshold was not provided for the "% Whole Panel Covered at 138X" metric for the analysis of gene amplifications on the PanSolid CLC pipeline. A coverage downsampling experiment was performed to assess the how coverage impacted accuracy for detecting gene amplifications and deletions.

# Methods

FASTQ files were downsampled to different levels to assess how coverage impacts CNV calling for 5 samples. 1 sample was the SeraSeq +12 copies control, and the other 4 samples had previous results from WGS testing.
...
However, on WGS BMPR1A has a copy number of 2, which should show up as a relative loss on the PanSolid analysis. BMPR1A is not listed as a deletion at any coverage level, whilst other genes are listed as deleted which are not deleted in the WGS data.
...
The RAD51D and BRCA1 deletions were not detected on the "coarse" setting when the coverage was reduced. However, WGS reported an NCC of only 32.5% for this sample, so any heterozygous deletion would not be expected to be detected by the PanSolid pipeline.
...
It has a loss of chromosome 22, similar to sample 24035924. However, for sample 24042074 the loss of CHEK2 was not detected on PanSolid.
...
![Chromosome 22 loss detection for samples 24035924 and 24042074](%60r%20paste0(data_folder,%20%22live_service/INC9967_downsampling/images/%22,%20%22chr22_loss_detection.png%22)%60){#fig-chr22-detection}
...
-   Reducing coverage as low as 30% at 138X did not significantly impact whole-gene fold changes of coordinates for genes with copy numbers of 0 on WGS (CN0). These samples had signal-adjusted noise from 0.2 to 0.8.
...
-   One sample from the deletions cohort (24042074) had a chr22 loss which was not detected on PanSolid when the noise was 0.58 and the percent coverage at 138X was 64.9%. This indicates that a combination of noise above 0.58 and percent 138X above 65% would be needed to detect CN1 deletions.
...
Long term, it may be worth using a different coverage metric for quality control. We have seen samples with 0% coverage at 138X but acceptable signal-adjusted noise (WS148333, 24068563).
